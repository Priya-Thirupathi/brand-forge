...
- No paid APIs. Gemini free tier only. [D2]
...
5. As a user, I'm told upfront that submitted ideas are stored, publicly visible, and processed by Gemini's free tier, so I don't enter anything confidential. [D6]
...
- Guardrails: banned words and Gemini safety feedback on input; on output, schema shape, lengths and counts, banned words, regulated health/certification claims, material-claim consistency, famous-brand collisions, leaked placeholders. One quality retry with feedback per step. [D11, D17, D22]
...
**Stage 3 — Deployment**
- Vercel + Supabase-hosted Postgres; protections verified; global cap sized to the project's quota.
...
| M6 | Billing exposure | No billing account on the Gemini project; the global cap is ≤ the project's daily quota ÷ 6 | Configuration checklist |
...
| Free-tier content may be used by Google to improve its products | Disclosed on the Generate tab; the demo is framed as public |
| LLM output variability | Per-step quality retry with feedback; 3 name candidates; repeats and confidence intervals in eval |
| LLM judge bias (Gemini judging Gemini) | Pinned judge model, preferably a different generation than the generator; deterministic metrics alongside; Stage 5 calibration against human labels |
...
| Offensive content passing guardrails into the public gallery | Gemini safety filters; manual `hidden` flag; a new rule for each case found |
