...
| regenerated_from_run_id | uuid, fk → runs.id, nullable | set when this run rebuilt the copy around another name from that run's candidates [D30] — kept apart from the column above, which means a *failure* retry |
...
{ idea: string; category: string; feasibility_option_id?: string;   // option defaults to the category's default
  ...
  regenerate_from_run_id?: string; alternate_name?: string }  // [D30] both or neither; skips naming, pins it to that run's
                                  //       candidate, 400 unless the name still passes the per-candidate name rules
```
...
- **Fixture:** 22 cases, versioned (14 normal covering every category; 6 adversarial: 2 prompt injections, banned-word bait, regulated-claim bait, material bait, a safety-blocked idea; 2 consistency [D31]). Expected outcomes `pass`, `reject`, or `safe` (rejected, or succeeded with relevance ≥ 0.5).
- **Consistency cases [D31]:** carry `follow_up_to`, the id of an earlier case whose brand they attach to as a D29 follow-up. Scored on `tone_fit` from a second judge call — does the *copy* read in the inherited voice — and deliberately not on distinctiveness or name uniqueness, since the name was produced by the case they inherit from. An unresolvable prerequisite records an `error` row and calls nothing.
...
- **Judge:** `JUDGE_MODEL` pinned and recorded, preferably a different model generation than the generator; rubric adapted from the `prior-eval-prototype` judge. A second, separate prompt scores `tone_fit` on consistency cases only [D31].
